...
HTML;
	...
	function getBuffer(&$data)
	{
		...
		if ($action == 'login2') {
			...
			//$username = \JFusion\Core\Factory::getApplication()->input->get('user');
			//$password = \JFusion\Core\Factory::getApplication()->input->get('hash_passwrd');
			//get the userinfo directly from SMF
			//$JFusionUser = \JFusion\Core\Factory::getUser($this->getJname());
...
//        $regex_body[] = '#action="' . $data->integratedURL . 'index.php(.*?)"(.*?)>#m';
	...
	function fixAction($matches)
	{
		...
		$baseURL = $this->data->baseURL;
		//\JFusion\Core\Framework::raise(LogLevel::WARNING, $url, $this->getJname());
	...
	function fixRedirect($matches) {
		...
		//\JFusion\Core\Framework::raise(LogLevel::WARNING, htmlentities($return), $this->getJname());
		return $return;
	}
