...
class executeJFusionHook
{
    ...
    function __construct($hook, &$vars, $key = '')
    {
        ...
        if (!defined('_JFVB_PLUGIN_VERIFIED') && $hook != 'init_startup' && defined('_VBJNAME') && defined('_JEXEC') && empty($_POST['logintype'])) {
	        ...
	        $user = \JFusion\Core\Factory::getUser(_VBJNAME);
    ...
    function global_complete()
    {
        if (defined('_JEXEC')) {
            ...
            if ($vbulletin->userinfo['userid'] != 0 && empty($vbulletin->GPC[COOKIE_PREFIX . 'userid'])) {
                ...
                $cookies = \JFusion\Core\Factory::getCookies();
            ...
            */
	        ...
	        if (defined('_VBJNAME')) {
		        try {
			        $vdb = \JFusion\Core\Factory::getDatabase(_VBJNAME);
        ...
        if (!empty($_POST['jfvbtask'])) {
            ...
            if ($vbsefenabled == 1) {
                if ($vbsefmode == 1) {
	                ...
	                $jfile = \JFusion\Core\Factory::getApplication()->input->get('jfile', false);
    ...
    function header_redirect()
    {
        ...
        if (strpos(strtolower($this->vars['url']), strtolower($testURL)) === false) {
            ...
            $url = JFusionFunction::routeURL($url, \JFusion\Core\Factory::getApplication()->input->getInt('Itemid'));
            $this->vars = $url;
        }
        //convert &amp; to & so the redirect is correct
        $this->vars = str_replace('&amp;', '&', $this->vars);
    ...
    function member_profileblock_fetch_unwrapped()
    {
        ...
        if (!empty($this->vars['profileurl']) && $profileurlSet !== true) {
            $uid = \JFusion\Core\Factory::getApplication()->input->get('u');
    ...
    function xml_print_output()
    {
        ...
        //parse AJAX output
	    if (defined('_VBJNAME')) {
		    $platform = \JFusion\Core\Factory::getPlayform('Joomla', _VBJNAME);
		    $params = \JFusion\Core\Factory::getParams(_VBJNAME);
